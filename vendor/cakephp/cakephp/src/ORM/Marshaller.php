...
class Marshaller {
        ...
        foreach ($include as $key => $nested) {
            ...
            if (isset($options['isMerge'])) {
                $callback = function ($value, $entity) use ($assoc, $nested) {
                    ...
                    return $this->_mergeAssociation($entity->get($assoc->getProperty()),
                                    $assoc, $value, $options);
                };
            }
            else {
    ...
    protected function _validate($data, $options, $isNew)
    {
        ...
        if ($validator === null) {
            ...
            sprintf('validate must be a boolean, a string or an object. Got %s.',
    ...
    protected function _prepareDataAndOptions($data, $options)
    {
        ...
        $this->_table->dispatchEvent('Model.beforeMarshal',
     ...
     */
    protected function _belongsToMany(BelongsToMany $assoc, array $data,
            $options = [])
    {
        ...
        if (!empty($conditions)) {
            ...
            foreach ($data as $i => $row) {
                ...
                if (isset($existing[$key])) {
                    $records[$i] = $this->merge($existing[$key], $data[$i],
                            $options);
                }
            }
        }
        ...
        foreach ($records as $i => $record) {
            ...
            if (isset($data[$i]['_joinData'])) {
                $joinData = $jointMarshaller->one($data[$i]['_joinData'],
                        $nested);
     ...
     * @see \Cake\ORM\Entity::$_accessible
     */
    public function merge(EntityInterface $entity, array $data,
            array $options = [])
    {
        list($data, $options) = $this->_prepareDataAndOptions($data, $options);
        $isNew = $entity->isNew();
    ...
    protected function _mergeAssociation($original, $assoc, $value, $options)
    {
        ...
        if ($assoc->type() === Association::MANY_TO_MANY) {
            return $marshaller->_mergeBelongsToMany($original, $assoc, $value,
                            (array) $options);
        }
    ...
    protected function _mergeJoinData($original, $assoc, $value, $options)
    {
        ...
        foreach ($records as $record) {
            ...
            if (isset($extra[$hash])) {
                $record->set('_joinData',
                        $marshaller->merge($extra[$hash], $value, $nested));
            }
