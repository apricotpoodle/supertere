...
class Table implements RepositoryInterface, EventListenerInterface, EventDispatcherInterface,
    ...
    public function getEntityClass()
    {
        if (!$this->_entityClass) {
            ...
            $alias = Inflector::classify(Inflector::underscore(substr(array_pop($parts),
                                            0, -5)));
    ...
    public function getBehavior($name)
    {
        ...
        if ($behavior === null) {
            throw new InvalidArgumentException(sprintf(
                    'The %s behavior is not defined on %s.', $name,
                    get_class($this)
            ));
        }
    ...
    public function belongsTo($associated, array $options = [])
    {
        ...
        $association = $this->_associations->load(BelongsTo::class, $associated,
                $options);
    ...
    public function hasOne($associated, array $options = [])
    {
        ...
        $association = $this->_associations->load(HasOne::class, $associated,
                $options);
    ...
    public function hasMany($associated, array $options = [])
    {
        ...
        $association = $this->_associations->load(HasMany::class, $associated,
                $options);
    ...
    public function belongsToMany($associated, array $options = [])
    {
        ...
        $association = $this->_associations->load(BelongsToMany::class,
                $associated, $options);
    ...
    public function findThreaded(Query $query, array $options)
    {
        ...
        $options = $this->_setFieldMatchers($options,
                ['keyField', 'parentField']);

        return $query->formatResults(function ($results) use ($options) {
                    /** @var \Cake\Collection\CollectionInterface $results */
                    return $results->nest($options['keyField'],
                                    $options['parentField'],
                                    $options['nestingKey']);
                });
    }
    ...
    public function get($primaryKey, $options = [])
    {
        ...
        if (count($key) !== count($primaryKey)) {
            ...
            throw new InvalidPrimaryKeyException(sprintf(
                    ...
                    $this->getTable(), implode(', ', $primaryKey)
            ));
        }
        ...
        if ($cacheConfig) {
            if (!$cacheKey) {
                $cacheKey = sprintf(
                        'get:%s.%s%s', $this->getConnection()->configName(),
                        $this->getTable(), json_encode($primaryKey)
                );
            }
            ...
            $options = [])
    {
        ...
        $entity = $this->_executeTransaction(function () use ($search, $callback, $options) {
            return $this->_processFindOrCreate($search, $callback,
     ...
     * @throws \Cake\ORM\Exception\PersistenceFailedException When the entity couldn't be saved
     */
    protected function _processFindOrCreate($search, callable $callback = null,
            $options = [])
    {
        ...
        if ($options['defaults'] && is_array($search)) {
            $accessibleFields = array_combine(array_keys($search),
                    array_fill(0, count($search), true));
            $entity = $this->patchEntity($entity, $search,
    ...
    public function save(EntityInterface $entity, $options = [])
    {
        ...
        if ($success) {
            if ($this->_transactionCommitted($options['atomic'],
                            $options['_primary'])) {
                $this->dispatchEvent('Model.afterSaveCommit',
                        compact('entity', 'options'));
            }
    ...
    protected function _processSave($entity, $options)
    {
        ...
        $saved = $this->_associations->saveParents(
                $this, $entity, $options['associated'],
    ...
    protected function _insert($entity, $data)
    {
        ...
        $filteredKeys = array_filter($primary,
                function ($v) {
            return $v !== null;
        });
        $data += $filteredKeys;
            ...
            foreach ($primary as $k => $v) {
                if (!isset($data[$k]) && empty($schema->getColumn($k)['autoIncrement'])) {
                    ...
                    $msg .= sprintf(
                            'Got (%s), expecting (%s)',
                            implode(', ',
                                    $filteredKeys + $entity->extract(array_keys($primary))),
    ...
    protected function _update($entity, $data)
    {
        ...
        if (!$entity->has($primaryColumns)) {
            ...
            $message .= get_class($entity) . ' is missing ' . implode(', ',
                            $primaryColumns);
            throw new InvalidArgumentException($message);
        }
    ...
    public function delete(EntityInterface $entity, $options = [])
    {
        ...
        if ($success && $this->_transactionCommitted($options['atomic'],
                        $options['_primary'])) {
