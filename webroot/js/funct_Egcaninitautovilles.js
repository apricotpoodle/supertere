...
function nomController(suite = '') {
    ...
    // return  suite;
    return getWebRoot() + 'Egcaninitautovilles/' + suite;
}
/**
...
function initAutoVilleApply() {
    var date = $('#yearpicker').val();
    var splt = date.split("/"); // split on "/"
    var year = splt[2]; // le millésime

    var isrc = $('#cbindiCle').combobox('getValue');
    // $.messager.alert('Info', 'Bravo ! :: ' + year + "::" + isrc);
    window.location.href = nomController("InitAutoVille/" + year + "/" + isrc);
    //
    //var year = "2222";
    /*
     var $rows = dgg_getSelections();
